...
export function docsPage(): Response {
  return page(
    ...
    bar() +
      ...
      `<section id="install"><h2>Install &amp; update</h2><div class="code"><i>$ </i><b>curl https://makarima.xyz/install.sh | sh</b></div><p>Installs a single static <code>mt</code> binary to <code>~/.local/bin</code> (override with <code>INSTALL_DIR</code>). Linux and macOS, amd64 and arm64. <code>mt update</code> verifies a signed release before installing it; <code>mt version</code> prints the installed version. Release Ed25519 public key: <code>B12X/7YmLu4Oc2eR1IKgywrVZAD2EVWJepcIONMBdTI=</code> (SHA-256: <code>09453b5ffd77ce0e6a5e39cebb9a9773cf0a3c97a540423e6eb1ffc27dcfffa8</code>).</p></section>` +

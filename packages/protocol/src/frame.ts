...
export function encodeFrame(
  ...
  if (requestId.length !== REQUEST_ID_SIZE) {
    throw new ProtocolError({
      code: "invalid_header",
      message: `Request id must be ${String(REQUEST_ID_SIZE)} bytes, got ${String(requestId.length)}`,
    });
  }
  if (payload.length > MAX_FRAME_PAYLOAD_BYTES) {
    throw new ProtocolError({
      code: "payload_too_large",
      message: `Payload of ${String(payload.length)} bytes exceeds max of ${String(MAX_FRAME_PAYLOAD_BYTES)} bytes`,
    });
  }
...
export function decodeFrame(data: Uint8Array): DecodedFrame {
  if (data.length < HEADER_SIZE) {
    throw new ProtocolError({
      code: "invalid_header",
      message: `Frame too short: expected at least ${String(HEADER_SIZE)} bytes, got ${String(data.length)}`,
    });
  }
  ...
  if (typeByte === undefined || !isValidFrameType(typeByte)) {
    throw new ProtocolError({
      code: "unknown_frame_type",
      message: `Unknown frame type: ${String(typeByte)}`,
    });
  }
  ...
  if (payloadLength > MAX_FRAME_PAYLOAD_BYTES) {
    throw new ProtocolError({
      code: "payload_too_large",
      message: `Declared payload length ${String(payloadLength)} exceeds max of ${String(MAX_FRAME_PAYLOAD_BYTES)} bytes`,
    });
  }
  ...
  if (payloadLength !== actualPayloadLength) {
    throw new ProtocolError({
      ...
      message: `Declared payload length ${String(payloadLength)} does not match actual remaining bytes ${String(actualPayloadLength)}`,
    });
  }
